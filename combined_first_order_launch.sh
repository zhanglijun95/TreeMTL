...
########### others ##########
exp_dir=2task_mobilenet_1206/
seed=10
# backbone='resnet34' # 5 (coarse) = 17 = all share
backbone='mobilenet' # 9/6 (coarse) = 32 = all share
reload=false
...
for taskcombination in "${taskcombinations[@]}"; do
    ...
    task1=${two_task[0]}
    task2=${two_task[1]}
    for ((branch=0;branch<=9;branch++)); do 
       if ((exp_i>=20)); then
       ...
       if ${reload}; then
           ...
           sbatch --partition ${partition} --job-name=FO${branch} \
            ...
            --exclude node[069,029,030,051,059,083,084,095,104,124,176] \
