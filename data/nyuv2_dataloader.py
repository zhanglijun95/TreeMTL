...
class NYU_v2(torch.utils.data.Dataset):
    ...
    def __scale__(self, img, label, task):
        """
    ...
    def __random_crop_and_pad_image_and_labels__(self, img, label, task, ignore_label=255):
        # combining
        label = label.astype('float32')
        if task == 'segment_semantic' or task == 'normal':
            label -= ignore_label
    ...
    def __scale__val__(self, img, label, task):
    ...
    def __getitem__(self, idx):
        ...
        for task in self.task:
            ...
            if self.mode == 'train':
                img, label = self.__scale__(img, label, task)
                img, label = self.__mirror__(img, label)
                img, label = self.__random_crop_and_pad_image_and_labels__(img, label, task)
            elif self.mode in ['test', 'val']:
                img, label = self.__scale__val__(img, label, task)
