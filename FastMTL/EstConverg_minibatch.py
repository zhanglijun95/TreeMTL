...
parser.add_argument('--batch_num', action='store', dest='batch_num', default=10, type=int, help='batch number for estimation')
parser.add_argument('--lr', action='store', dest='lr', default=0.001, type=float, help='learning rate')

# Rerun layouts
parser.add_argument('--rerun', action='store', nargs='+', dest='rerun', default=None, type=int, help='rerun some layouts')
...
if args.data == 'NYUv2':
    ...
    trainDataloaderFix = DataLoader(dataset, args.bz, shuffle=True, worker_init_fn=seed_worker, generator=g) 
...
total_iters, short_iters, start, step, batch_num = args.total_iters, args.short_iters, args.start, args.step, args.batch_num
...
for L in layout_list:
    ...
    if args.rerun is not None:
        if not layout_idx in args.rerun:
            continue
    if layout_idx == 2:
        layout_est.append({})
        continue
            
    print('Layout {}'.format(layout_idx), flush=True)
